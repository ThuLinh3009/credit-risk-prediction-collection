...
├── notebooks/          # 01_credit_risk_pipeline.ipynb — demo end-to-end (EDA -> priority ranking)
├── papers/             # Paper gốc Yeh & Lien (2009) và tài liệu tham khảo
├── presentations/      # Slide thuyết trình
├── reports/            # Kết quả sinh bởi src/pipeline.py (metrics, so sánh paper gốc, SHAP, priority ranking)
├── src/                # Mã nguồn pipeline: data.py, features.py, models.py, evaluate.py, interpret.py, priority.py
└── README.md
```
...
Lệnh trên chạy toàn bộ pipeline (làm sạch dữ liệu → feature engineering → huấn luyện &
tinh chỉnh 4 mô hình → đánh giá → SHAP → xếp hạng ưu tiên thu hồi nợ) và ghi kết quả vào
`reports/` (`model_metrics.csv`, `comparison_with_paper_2009.csv`, `shap_top_features.csv`,
`priority_ranking.csv`). Notebook `notebooks/01_credit_risk_pipeline.ipynb` trình bày lại
cùng pipeline theo từng bước, kèm biểu đồ.

**Kết quả hiện tại** (chạy thử trên tập test, xem chi tiết trong `reports/`):

| Model | Test AUC-ROC | Ghi chú |
|---|---|---|
| XGBoost | ~0.79 | Không có trong paper gốc (kỹ thuật hiện đại hơn) |
| Random Forest | ~0.78 | Không có trong paper gốc |
| Neural Network (MLP) | ~0.78 | Paper gốc (quy đổi từ area ratio): AUC ≈ 0.77 |
| Logistic Regression | ~0.74 | Paper gốc (quy đổi từ area ratio): AUC ≈ 0.72 |

Số liệu paper gốc là quy đổi gần đúng từ area ratio trong lift chart (Table 1, Yeh & Lien
2009) sang AUC-ROC (`AUC ≈ (area_ratio + 1) / 2`), không phải AUC-ROC do tác giả gốc trực
tiếp công bố — xem ghi chú chi tiết trong `src/config.py` và mục 3.2 của notebook.
...
```bash
git clone https://github.com/ThuLinh3009/credit-risk-prediction-collection.git
cd credit-risk-prediction-collection
python -m venv .venv
source .venv/bin/activate    # Windows: .venv\Scripts\activate
pip install -r requirements.txt
```

Xem mục [6.1. Chạy pipeline](#61-chạy-pipeline) để chạy toàn bộ pipeline hoặc mở notebook demo.
